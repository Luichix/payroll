CREATE TABLE `database_payroll`.`OVERTIME` (
  `ID_OVERTIME` INT NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `FK_OVERTIME_USER_ADD`
    FOREIGN KEY (`FK_USER_ADD`)
    REFERENCES `database_payroll`.`USER` (`ID_USER`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `FK_OVERTIME_USER_ALTER`
    FOREIGN KEY (`FK_USER_ALTER`)
    REFERENCES `database_payroll`.`USER` (`ID_USER`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `FK_OVERTIME_USER_LOW`
